...
const typeOptions = Object.entries(DIY_PROVIDERS).map<SelectOption>(
	([id, provider]) => ({
		label: provider.name,
		value: id,
...
const save = () => {
	...
	textAiForm.value.validate().then(
		({ warnings }) => {
...
const searchForModel = useThrottleFn(
	async (provider: TextProvider, search: string) => {
		try {
...
<template>
	<NCard>
		...
		>
			<NForm ref="textAiForm" :model="formValue" @submit.prevent="save">
				<NCard
					...
					:style="{
						backgroundColor: `oklch(0.3 0.05 ${provider.hue} / 50%)`
					}"
				>
					<template #header-extra>
						<NFlex>
							...
							<NPopconfirm
								...
								class="rpgm-app"
								@positive-click="
									formValue.providers?.splice(i, 1)
								"
							>
								<template #trigger>
									<NButton
					...
					<NFlex>
						...
						>
							<NSelect
								...
								@search="
									search =>
										void searchForModel(provider, search)
								"
							/>
						</NFormItemRow>
					</NFlex>
				...
				<NFlex vertical>
					...
					<NButtonGroup ref="saveButton" style="flex-wrap: wrap">
						<NButton
							type="primary"
							attr-type="submit"
							:disabled="saved"
