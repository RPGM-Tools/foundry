...
export class ChatWizard<T extends WizardData['data'] = WizardData['data']> {
	...
	get placeholder() {
		return `<div class="${this.moduleId}-${this.key}">If you are seeing this, please install ${this.moduleId}</div>`;
	}
	readonly _key: string;

	/** @returns The unique key for this wizard and user */
	get key(): ClientSettings.KeyFor<typeof this.moduleId> {
		return `${this._key}-${game.userId}` as unknown as ClientSettings.KeyFor<
			typeof this.moduleId
		>;
	}
	...
	render(message: ChatMessage, html: HTMLElement): boolean {
		...
		if (data === undefined) {
			this.logger.warn(
				`No data found for ${this.key} wizard with id ${id}, deleting...`
			);
			void message.delete({});
			return false;
		}
		...
		const mount = html.querySelector<HTMLElement>(
			`.${this.moduleId}-${this.key}`
		)!;
	...
	load() {
		...
		this.data = data
			? new Map(Object.entries(JSON.parse(data) as Record<string, T>))
			: new Map<string, T>();
		rpgm.chat.registerMessageHandler(this);
		this.logger.debug(
			`Loaded ${this.key} wizard with ${this.data.size} messages`
		);
	}
	...
	prune() {
		...
		this.data = new Map(
			[...this.data].filter(([id]) => message_ids.includes(id))
		);
		this.save();
		if (old_length > this.data.size)
			this.logger.debug(
				`Pruned ${this.key} wizard with ${old_length - this.data.size} messages`
			);
	}
